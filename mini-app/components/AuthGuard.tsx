...
export default function AuthGuard({ children }: AuthGuardProps) {
    ...
    if (isLoading) {
        return (
            <div className="fixed inset-0 bg-[#0a0e1a] flex flex-col items-center justify-center">
                ...
                <p className="text-white text-lg font-medium">Авторизация...</p>
                <p className="text-gray-500 text-sm mt-2">Проверка данных Telegram</p>
            </div>
        );
    }
    ...
    if (error && !isAuthenticated) {
        return (
            <div className="fixed inset-0 bg-[#0a0e1a] flex flex-col items-center justify-center px-6">
                ...
                <h1 className="text-white text-xl font-bold mb-2 text-center">Ошибка авторизации</h1>
    ...
    if (isAuthenticated && !isWhitelisted) {
        return (
            <div className="fixed inset-0 bg-[#0a0e1a] flex flex-col items-center justify-center px-6">
                <div className="text-6xl mb-6">🔒</div>
                <h1 className="text-white text-xl font-bold mb-2 text-center">Доступ запрещён</h1>
                <p className="text-gray-400 text-center mb-4">
                    Вы не имеете доступа к этому приложению.
                </p>
                {user && (
